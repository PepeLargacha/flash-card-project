...
class FlashCard(Canvas):
    ...
    def get_new_word(self, index_list, data, language):
        try:
            ...
            pass
        if len(index_list) == 0:
            messagebox.showinfo(title='Congratulations!!!', message='You Learn All the words. Well done.')
            self.parent_gui.finalize()
        else:
...
class MyGui(Tk):
    ...
    def grab_data(self):
        try:
            ...
            if len(self.words_index) < 2 or self.words_index == None:
                self.data = pandas.read_csv(f'data/{self.language.lower()}_words.csv')
                self.words_index = [i for i in self.data.index]
